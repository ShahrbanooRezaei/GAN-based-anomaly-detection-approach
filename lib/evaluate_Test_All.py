...
def performance(opt,epoch):
    ...
    LS1 = np.array(pd.read_csv(score_root_0, header=None))
    LS2 = np.array(pd.read_csv(score_root_00, header=None))
    ...
    label = np.array(pd.read_csv(label_root, header=None))
    ...
    thres = opt.threshold
    ##FS1
    per(thres,scores_FS1_norm, label[:,0],'FS1',epoch)
    roc(scores_FS1_norm, label[:,0],'FS1',epoch)
    PR(scores_FS1_norm, label[:,0],'FS1',epoch)
    
    ##FS2
    per(thres,scores_FS2_norm, label[:,0],'FS2',epoch)#!!!!!!!!!!!!!!
    roc(scores_FS2_norm, label[:,0],'FS2',epoch)#!!!!!!!!!!!!!!!
    PR(scores_FS2_norm, label[:,0],'FS2',epoch)
    
    ##lS1
    per(thres,scores_LS1_norm, label[:,1],'LS1',epoch)
    roc(scores_LS1_norm, label[:,1],'LS1',epoch)
    PR(scores_LS1_norm, label[:,1],'LS1',epoch)
    
    ##lS2
    per(thres,scores_LS2_norm, label[:,1],'LS2',epoch)#!!!!!!!!!!!!!!
    roc(scores_LS2_norm, label[:,1],'LS2',epoch)#!!!!!!!!!!!!!!!
    PR(scores_LS2_norm, label[:,1],'LS2',epoch)
