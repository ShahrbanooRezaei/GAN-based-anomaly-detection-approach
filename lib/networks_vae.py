...
class Encoder(nn.Module):
    ...
    def __init__(self, isize1, isize2, nz, nc, ndf, ngpu, add_final_conv):
...
model = Encoder(4, 245, 100, 1, 32, 1,0).to(device)
...
class NetD(nn.Module):
    ...
    def __init__(self, opt):
        ...
        model = Encoder(opt.isize1, opt.isize2, 1, opt.nc, opt.ngf, opt.ngpu, 1)
...
class NetG(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.encoder1 = Encoder(opt.isize1, opt.isize2, opt.nz, opt.nc, opt.ngf, opt.ngpu, 0)
        self.decoder = Decoder(opt.isize1, opt.isize2, opt.nz, opt.nc, opt.ngf, opt.ngpu)
        self.encoder2 = Encoder(opt.isize1, opt.isize2, opt.nz, opt.nc, opt.ngf, opt.ngpu,0)
        
        self.fc_mu = nn.Linear(opt.ndf*2*16, opt.nz)
        self.fc_var = nn.Linear(opt.ndf*2*16, opt.nz)
        
        self.fc_mu_o = nn.Linear(opt.ndf*2*16, opt.nz)
        self.fc_var_o = nn.Linear(opt.ndf*2*16, opt.nz)
        
        # self.decoder_input = nn.Linear(opt.nz, opt.ndf*2*16)
    ...
    def forward(self, x):
        ...
        result = z.view(-1, 100, 1, 1)
        
        gen_imag = self.decoder(result)
        
        result_o = self.encoder2(gen_imag)
        result_o = torch.flatten(result_o, start_dim=1)
        
        # Split the result into mu and var components
        # of the latent Gaussian distribution
        mu_o = self.fc_mu_o(result_o)
        log_var_o = self.fc_var_o(result_o)
        return mu, log_var, gen_imag, mu_o, log_var_o
