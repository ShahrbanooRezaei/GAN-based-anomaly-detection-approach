...
class BaseModel():
    ...
    def train(self):
        ...
        con_err_t = torch.zeros(size=(self.opt.niter, 1), dtype=torch.float32, device=self.device)
        tot_err_t = torch.zeros(size=(self.opt.niter, 1), dtype=torch.float32, device=self.device)
        
        con_err_v = torch.zeros(size=(self.opt.niter, 1), dtype=torch.float32, device=self.device)
        tot_err_v = torch.zeros(size=(self.opt.niter, 1), dtype=torch.float32, device=self.device)
        ##
        # TRAIN
        t = -1
        best_auc = 0
        ...
        for self.epoch in range(self.opt.iter, self.opt.niter):
            ...
            if self.epoch % 1 ==0 :
                ...
                # con_err , tot_err = self.test(self.dataloader_test,'Test') 
                # self.test(self.dataloader_test_MW,'Test_MW')
                # self.save_weights(self.epoch)
                # con_err_t[t,0] = con_err
                # tot_err_t [t,0] = tot_err 
                
                con_err , tot_err = self.test(self.dataloader_val,'val')
                con_err_v[t,0] = con_err
                tot_err_v [t,0] = tot_err
            
            if self.epoch % 5 ==0 :  
                # np.savetxt('experiments/%s/%s/performance/con_t_%d_NM.csv' % (self.opt.dataset,self.opt.tun_name,self.epoch), con_err_t.squeeze().cpu().numpy(), delimiter=',')
                # np.savetxt('experiments/%s/%s/performance/tot_t_%d_NM.csv' % (self.opt.dataset,self.opt.tun_name,self.epoch), tot_err_t.squeeze().cpu().numpy(), delimiter=',')
                
                np.savetxt('experiments/%s/%s/performance/con_v_%d_NM.csv' % (self.opt.dataset,self.opt.tun_name,self.epoch), con_err_v.squeeze().cpu().numpy(), delimiter=',')
                np.savetxt('experiments/%s/%s/performance/tot_v_%d_NM.csv' % (self.opt.dataset,self.opt.tun_name,self.epoch), tot_err_v.squeeze().cpu().numpy(), delimiter=',')
    ...
    def test(self, dataloader_t, phase):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(dataloader_t, 0):
                ...
                err_g_adv = torch.mean(torch.pow((self.netd(self.input)[1]-self.netd(self.fake)[1]), 2), dim=(1,2,3))
                ...
                err_g_con = torch.mean(scores[:,:,5])
                self.con_err = self.con_err + err_g_con
                
                self.tot_err = self.tot_err + torch.mean(err_g_adv) * self.opt.w_adv + torch.mean(err_g_con) * self.opt.w_con + torch.mean(err_g_enc) * self.opt.w_enc
        ...
        return self.con_err , self.tot_err
        ##
