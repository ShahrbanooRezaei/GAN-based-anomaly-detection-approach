...
from lib.evaluate_Test_run_all import performance #evaluate_Test_FS1_FS2_all #evaluate_Test #evaluate_Test_FS1_FS2_all
    ...
    def test(self, dataloader_t, phase):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(dataloader_t, 0):
                ...
                mu, log_var, self.fake, mu_o, log_var_o = self.netg(self.input)
                ...
                err_g_enc = 0# orch.mean(torch.pow((latent_i-latent_o), 2), dim=(1,2,3))
...
class Ganomaly(BaseModel):
    ...
    def __init__(self, opt, dataloader, scaler):
        ...
        if self.opt.isTrain:
            self.netg.train()
            self.netd.train()
            self.optimizer_d = optim.Adam(self.netd.parameters(), lr=self.opt.lr)
            self.optimizer_g = optim.Adam(self.netg.parameters(), lr=self.opt.lr)

    ##
    def forward_g(self):
        """ Forward propagate through netG
        """
        self.mu, self.log_var, self.fake, self.mu_o, self.log_var_o = self.netg(self.input)
    ...
    def backward_g(self):
        ...
        self.err_g_enc = 0.5 * torch.sum(self.log_var_o.exp() - self.log_var_o + self.mu_o.pow(2) - 1, dim=1).mean() #??? Lost this equation
        self.err_g_kl =  0.5 * torch.sum(self.log_var.exp() - self.log_var + self.mu.pow(2) - 1, dim=1).mean()
        self.err_g = self.err_g_adv * self.opt.w_adv + \
                     self.err_g_con * self.opt.w_con + \
                     self.err_g_enc * self.opt.w_enc + \
                     self.err_g_kl * self.opt.w_kld
